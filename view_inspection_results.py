...
def img_to_time(img):
    return float(img[img.rfind('_') + 1 : img.rfind('.')])
imgs = [f for f in listdir(INSPECTION_DIR) if f.endswith('.jpg') and 'cam_{}'.format(cam_i) in f]
...
for imgname in imgs:
    ...
    with open(path.join(INSPECTION_DIR, 'cam_{}_time_{}_looking.json'.format(cam_i, int(img_to_time(imgname))))) as f:
        looking_content = json.load(f)
    with open(path.join(INSPECTION_DIR, 'cam_{}_time_{}_touching.json'.format(cam_i, int(img_to_time(imgname))))) as f:
        touching_content = json.load(f)
    ...
    names = set([eval(x)[0] for x in touching_content.keys()])
    ...
    for name1, name2 in itertools.product(names, names):
        if name1 is name2 or hash(name1) < hash(name2): # to only iter each pair once
            continue
        touching = touching_content[str((name1, name2))]
    ...
    for name1, name2 in itertools.product(names, names):
        if name1 is name2:
            continue
        looking = looking_content[str((name1, name2))]
        looking_dist = -1
        if looking:
            looking, lookint_dist = looking_content[str((name1, name2))]
